...
; Programmers: Troy Reindl, Ellie Chang
; Program Purpose: LC-3 program that calculates and displays the minimum, maximum, and average grade of 5 test
; scores and displays the letter grade for each.
; DATE: 06/03/2025
...
WEL .STRINGZ "\nPlease enter 5 test scores (0-99): \n"

; Use appropriate labels and comments
    LD R0, LINER
    OUT

; Get 5 user inputs for 5 test scores and store in array
; Use subroutine calls and branching for control

; 1st test score
    JSR CONVERT_INPUT           ; Get user score input
    LEA R6, SCORES         ; Load address of SCORES array
...
CALCULATE_MAX
    LD R1, COUNT       ; Load the number of tests
    LEA R2, SCORES         ; Load address of SCORES array
    LD R4, SCORES         ; Load the first grade as the initial max
    ST R4, MAX_SCORE        ; Store the initial max grade
    ADD R2, R2, #1          ; Move to the next grade in the array
    LOOP_MAX LDR R5, R2, #0    ; Load the current grade
    NOT R4, R4              ; Negate the current max grade
    ADD R4, R4, #1
    ADD R5, R5, R4          ; Compare the current grade with the maxgrade
    BRp CURR_MAX               ; If the current grade is greater, update the max
    LEA R0, STRING_MAX             ; Load address of "Maximum Score: " message
    PUTS                    ; Print the message
    LD R3, MAX_SCORE         ; Load the max grade
    AND R1, R1, #0
    JSR PRINT_SCORE          ; Call subroutine to break the integer into digits and print it
    LD R0, SPACE
    OUT
    LD R0, LINER
    OUT
...
CALCULATE_MIN
    LD R1, COUNT  
    LEA R2, SCORES
    LD R4, SCORES          ; Load the first grade as the initial minimum
    ST R4, MIN_SCORE        ; Store the initial minimum grade
    ADD R2, R2, #1          ; Move to the next grade in array
    ADD R1, R1, #-1         ; Decrement the loop counter
    LOOP_MIN LDR R5, R2, #0    ; Load current grade
    ...
    ADD R2, R2, #1          ; Move to the next grade in the array
    LD R4, SCORES          ; Reset the minimum grade to the first grade
    AND R5, R5,#0
    ADD R1,R1,#-1           ; Decrement the loop counter
    ...
    PUTS                    ; Print the message
    LD R3, MIN_SCORE         ; Load the minimum grade
    AND R1, R1, #0
    JSR PRINT_SCORE           ; Call subroutine to break the integer into digits and print it
    LD R0, SPACE
    OUT
...
CALC_AVG
    ...
    ADD R3, R3, R4          ; Add the current grade to sum
    ADD R2, R2, #1          ; Move to the next grade in the array
    ADD R1, R1, #-1         ; Decrement the loop counter
    ...
    DONE_AVE
    ST R6, AVG_SCORE    ; Store the avg score
    LEA R0, STRING_AVG             ; Load address of "Average Score: " message
    PUTS                    ; Print the message
    AND R3, R3, #0
    AND R1, R1, #0
    AND R4, R4, #0
    ADD R3, R3, R6          ; Load the avg score
    JSR PRINT_SCORE           ; Call subroutine to break the integer into digits and print it

; Restart program if desired
    JSR RESTART_PROGRAM        ; Call subroutine to prompt the user to restart the program
    HALT
...
COUNT   .FILL #5
RESTART2 .FILL x3000
MAX_SCORE  .BLKW #1          ; Reserve memory for storing the maximum grade
MIN_SCORE   .BLKW #1          ; Reserve memory for storing the minimum grade
AVG_COMPLETE .BLKW #1
AVG_SCORE .BLKW #1      ; Reserve memory for storing the average score

; Branches for calculating min and max
CURR_MIN
LDR R4, R2, #0              ; Load the current grade
ST R4, MIN_SCORE              ; Update the minimum grade
ADD R2, R2, #1              ; Move to the next grade in the array
ADD R1, R1, #-1             ; Decrement the loop counter
BRnzp LOOP_MIN                 ; Continue the loop
CURR_MAX
LDR R4, R2, #0              ; Load the current grade
ST R4, MAX_SCORE             ; Update the maximum grade
ADD R2, R2, #1              ; Move to the next grade in the array
ADD R1, R1, #-1             ; Decrement the loop counter
BRp LOOP_MAX                   ; Continue the loop

; Array to store test scores
SCORES .BLKW #5             ; Reserve memory for storing 5 test scores
...
LD R0, LINER
OUT
LEA R0 RESTARTPROG_STR      ; output prompt
...
RESTARTPROG_STR .STRINGZ  "Would you like to run this program again?\n "
...
JSR VALIDATE_DATA                  ; Call subroutine to validate the input
OUT                         ; Echo the input
...
VALIDATE_DATA ST R1, REGVAL3      ; Save the value of R1
